...
**API 限流优化**
- 429 重试退避策略增强：从 1/2/4/8s 改为 5/10/20/40s，更稳健地应对飞书频控
- `batch-create-docs`：文档创建间自动间隔 3 秒，开始前提示用户预估耗时
- `bitable-update` / `bitable-write`：批次间自动间隔 3 秒
- 新增导出 `sleep()` 和 `API_INTERVAL_MS`（3s）工具函数，供批量脚本使用

**文档更新**
- SKILL.md：多文档操作从「并行 sub-agent」改为「串行 + 间隔」，新增 API 限流注意事项
- technical.md：更新限流重试公式和批量操作间隔说明
